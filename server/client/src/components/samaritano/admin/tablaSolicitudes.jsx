...
const TablaDatos = ({ setValoresForm, newFilter, setFiltro, setModalUpdate,
  ...
  const [eliPeticion, setEliPeticion] = useState('');
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className=''>
                  <tr className="font-bold text-gray-900">
                    ...
                    <th className="px-4 py-2">¿Tiene alguna discapacidad?</th>
                    <th className="px-4 py-2">Solicitud de</th>
                    <th className="px-4 py-2">Petición</th>
                  ...
                  {partirData.length > 0 ?
                    ...
                    partirData.map((e) => (
                      <tr key={e.id} className="hover:bg-gray-100 border-b border-gray-300">
                        ...
                        <td className="py-3">{e.apellidos}</td>
                        <td className="py-3">{e.edad}</td>
                        <td className="py-3">{e.estadocivil}</td>
                        <td className="py-3">{e.discapacidad}</td>
                        <td className="py-3">{e.solicitud}</td>
                        <td className="py-3">{e.peticion}</td>
                        ...
                        <td className="py-3 text-black-600 hover:bg-blue-100">
                          ...
                          {
                            elimf ? (
                              <button className="max-w-max my-auto h-min rounded-md bg-red-600 px-3 py-2 text-lg text-white shadow-md font-medium" onClick={() => eliminarDato(e.id, e.peticion)}>
  ...
  const eliminarDato = (id, peticion) => {
    setEliPeticion(peticion);
    setItemToDelete(id);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      await fetch(`${globalURL}peticiones/${itemToDelete}`, {
        method: 'DELETE',
      });
      ...
      {showWarningModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-modal">
          <div className="bg-white sm:mx-5 sm:w-96 p-5 rounded-xl shadow-mfBoxShadow border mx-2">
            <p>¿Estás seguro de que deseas eliminar <span className='font-semibold'>{eliPeticion}</span>?</p>
